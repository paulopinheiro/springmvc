package br.com.paulopinheiro.springmvc.persistence.repositories.springjdbc;

import br.com.paulopinheiro.springmvc.persistence.entities.User;
import br.com.paulopinheiro.springmvc.persistence.rowmappers.UserRowMapper;
...
public class UserDao {
    ...
    public int addSampleUser() {
        return jdbcTemplate.update("INSERT INTO user (first_name, last_name, enabled) VALUES (?, ?, ?)", "John", "Doe", 1);
    }

    public String getLastNameOfUserById(Integer id) {
        SqlParameterSource namedParameters = new MapSqlParameterSource().addValue("id", id);
        return namedParameterJdbcTemplate.queryForObject("SELECT last_name FROM user WHERE ID = :id", namedParameters, String.class);
    }
   ...
   public User getUserById(Integer id) {
    ...
    public int addSampleUserWithSimpleJdbcInsertDemo() {
        SimpleJdbcInsert simpleJdbcInsert = new SimpleJdbcInsert(dataSource).withTableName("user_test");
...
//	    simpleJdbcInsert.executeAndReturnKey(parameters);
    ...
    public User getUserWithFirstLastNameByIdFromStoredProcedure(Integer id) {
        SimpleJdbcCall simpleJdbcCall = new SimpleJdbcCall(dataSource).withProcedureName("SelectFirstLastNameByID");
